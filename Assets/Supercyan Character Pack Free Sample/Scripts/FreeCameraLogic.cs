...
public class FreeCameraLogic : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        Vector3 currentRotationX = new Vector3(30, currentRotationAngle, 0);


        Vector3 position = m_currentTarget.position;
        position -= currentRotation * Vector3.forward * m_distance;
        // position -= Vector3.back * m_distance * currentRotationAngle;
        position.y = targetHeight;

        transform.position = position;
        // transform.Rotate(currentRotationX);
        transform.LookAt(m_currentTarget.position + new Vector3(0, 2f, 0));
    }
}
